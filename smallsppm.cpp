...
public:
	...
	void update(HitPointKDTreeNode *p, Vec photon_pos, const Vec fl, const Ray r, const Vec n, const Object &obj)
	{
		if (!p)
			return;
		double mind = 0;
		if (photon_pos.x > p->max.x)
			mind += pow((photon_pos.x - p->max.x), 2);
		if (photon_pos.x < p->min.x)
			mind += pow((p->min.x - photon_pos.x), 2);
		if (photon_pos.y > p->max.y)
			mind += pow((photon_pos.y - p->max.y), 2);
		if (photon_pos.y < p->min.y)
			mind += pow((p->min.y - photon_pos.y), 2);
		if (photon_pos.z > p->max.z)
			mind += pow((photon_pos.z - p->max.z), 2);
		if (photon_pos.z < p->min.z)
			mind += pow((p->min.z - photon_pos.z), 2);
		if (mind > p->maxr2)
			return;
...
void trace(const Ray &r, int dpt, bool eye_ray, const Vec &fl, const Vec &throughput, HPoint *hitp = nullptr, bool emissive = true)
{
	...
	if (eye_ray)
	{
		...
		else
		{
			...
			if (brdf_f.x == 0.0 && brdf_f.y == 0.0 && brdf_f.z == 0.0)
				return;
			if (!brdf->isSpecular())
				hitp->direct = hitp->direct + throughput.mul(lightSampleDirect(r, x, n, id, obj));
