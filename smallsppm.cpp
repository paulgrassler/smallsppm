...
class BRDF_SPEC : public BRDF {
	public:
		...
		void evaluateBRDF(const Vec &n, const Vec d, const Vec &c, const Vec &new_d, Vec *brdf_f) override
		{
			...
			if (abs(perf_refl.dot(new_d) - perf_refl.length() * new_d.length()) < EPS)
			{
...
class BRDF_REFR : public BRDF {
	...
	public:
		...
		void evaluateBRDF(const Vec &n, const Vec d, const Vec &c, const Vec &new_d, Vec *brdf_f) override
		{
			...
			if (abs(perf_refl.dot(new_d) - perf_refl.length() * new_d.length()) < EPS)
			{
				*brdf_f = Vec(P, P, P).mul(c) / abs(n.dot(new_d));
			}
			else if (abs(perf_trans.dot(new_d) - perf_trans.length() * new_d.length()) < EPS)
			{
				*brdf_f = Vec(1 - P, 1 - P, 1 - P).mul(c) / abs(n.dot(new_d));
			}
			else
			{
				*brdf_f = Vec(0 ,0, 0);
...
std::vector<Triangle *> createBlock(float width, float height, float depth, bool without_top = false)
{
	...
	float h = height / 2;
	float d = depth / 2;
	std::vector<Triangle *> block = {
		new Triangle(Vec(-w, -h, d), Vec(w, -h, d), Vec(w, h, d)),//Front
		new Triangle(Vec(-w, -h, d), Vec(w, h, d), Vec(-w, h, d)),//Front
		new Triangle(Vec(w, -h, d), Vec(w, -h, -d), Vec(w, h, -d)),//Right
		new Triangle(Vec(w, -h, d), Vec(w, h, -d), Vec(w, h, d)),//Right
		new Triangle(Vec(w, -h, -d), Vec(-w, -h, -d), Vec(-w, h, -d)),//Back
		new Triangle(Vec(w, -h, -d), Vec(-w, h, -d), Vec(w, h, -d)),//Back
		new Triangle(Vec(-w, -h, -d), Vec(-w, -h, d), Vec(-w, h, d)),//Left
		new Triangle(Vec(-w, -h, -d), Vec(-w, h, d), Vec(-w, h, -d)),//Left
		new Triangle(Vec(-w, -h, -d), Vec(w, -h, -d), Vec(w, -h, d)),//Bottom
		new Triangle(Vec(-w, -h, -d), Vec(w, -h, d), Vec(-w, -h, d)),//Bottom
		new Triangle(Vec(-w, h, d), Vec(w, h, d), Vec(w, h, -d)),//Top
		new Triangle(Vec(-w, h, d), Vec(w, h, -d), Vec(-w, h, -d)),//Top
	};

	if (without_top)
	{
		delete block.at(block.size() - 1);
		block.pop_back();
		delete block.at(block.size() - 1);
		block.pop_back();
	}
...
std::vector<Triangle *> createRectangle(float width, float height, float depth, bool flip = false)
{
	float w = width / 2.0;
	float h = height / 2.0;
	float d = depth / 2.0;
	...
	{
		std::vector<Triangle *> rect = {
			new Triangle(Vec(-w, 0, -d), Vec(w, 0, -d), Vec(w, 0, d)),//Bottom
			new Triangle(Vec(-w, 0, -d), Vec(w, 0, d), Vec(-w, 0, d)),//Bottom
		};
	...
	{
		std::vector<Triangle *> rect = {
			new Triangle(Vec(-w, 0, d), Vec(w, 0, d), Vec(w, 0, -d)),//Top
			new Triangle(Vec(-w, 0, d), Vec(w, 0, -d), Vec(-w, 0, -d)),//Top
		};
...
{
	if (hit_obj.mat.brdf->getReflType() != PHONG)
	{
		printf("lightSampleDirect() called on non-PHONG BRDF\n");
		exit(1);
	}
...
void trace(const Ray &r, int dpt, bool eye_ray, const Vec &fl, const Vec &throughput, HPoint *hitp = nullptr, bool emissive = true)
{
	...
	if (eye_ray) {
		if (!brdf->isSpecular() && !brdf->isGlossy()) //only create a hitpoint if non-specular material
		{
			...
			{
				if (emissive) hitp->direct = hitp->direct + throughput.mul(obj.e);
				hitp->valid = false;
			}	
		...
		else
		{
			...
			if (pdf == 0.0)
			{
				//printf("WARNING: pdf == 0.0\n");
				return;
			}
			Vec brdf_f;
			brdf->evaluateBRDF(n, r.d, f, new_d, &brdf_f);	
			if (brdf_f.x == 0.0 && brdf_f.y == 0.0 && brdf_f.z == 0.0) return;
			if (!brdf->isSpecular()) hitp->direct = hitp->direct + throughput.mul(lightSampleDirect(r, x, n, id, obj));
	...
	else 
	{
		...
		if (brdf_f.x == 0.0 && brdf_f.y == 0.0 && brdf_f.z == 0.0) return;
		if (rand01()<p) trace(Ray(x,new_d, r.time),dpt,eye_ray, fl.mul(brdf_f) * (1./p) * abs(n.dot(new_d)) * (1.0 / pdf),throughput);
	}
}
